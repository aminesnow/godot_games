...
func save(idx):
	...
	save_data["quest_system"] = QuestSystem.get_save_data()
...
func load_save(idx):
	...
	if file.file_exists(_get_save_path(idx)):
		...
		#print(data)
		PlayerInventory.load_save_data(data["player_inventory"])
		ContainersInventory.load_save_data(data["containers_slots"])
